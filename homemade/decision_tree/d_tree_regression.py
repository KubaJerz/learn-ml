...
import math
import numpy as np
from decision_tree.tree_node_class import TreeNode
import pandas as pd

class DecisionTree():

    def __init__(self, max_depth = 5, min_samples = 1, min_info_gain = 0.0, criterion = 'mse', num_rand_features = -1) -> None:
    ...
    def __build_tree(self, data: np.array, curr_depth: int) -> TreeNode:
        ...
        s_data_left, s_data_right, s_f_idx, s_f_val, split_mae_or_mse = self.__find_best_split(data) #find the ebst split and info form the split

        if criterion == 'mae':
            curr_mae = self.__mae(data[:, -1]) #gets the entropy of the curr data 
            info_gain = curr_mae - split_mae_or_mse #calc the information gain from the best split on this data
        else:
            curr_mse = self.__mse(data[:, -1]) #gets the gini of the curr data 
            info_gain = curr_mse - split_mae_or_mse #calc the information gain from the best split on this data
